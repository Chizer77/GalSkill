...
def process_jawiki_content(content: str) -> str:  # noqa: PLR0915
    ...
    for template in wikicode.filter_templates():
        ...
        template_name_str = str(template.name).strip()
        try:
            match template_name_str:
                case "R" | "Refnest" | "refnest" | "Sfn" | "efn" | "Efn2" | "efn2" | "ISBN2" | "Anchors" | "anchors":
                    template_plain_text = ""
                case "仮リンク" | "en":
                    template_plain_text = str(template.get(1).value) if template.has(1) else ""
                case "要出典範囲":
                    template_plain_text = str(template.get("1").value) if template.has("1") else (str(template.get(1).value) if template.has(1) else "")
                case "Visible anchor" | "Vanc":
                    template_plain_text = str(template.get(1).value) if template.has(1) else ""
                case "読み仮名" | "Ruby" | "ruby" | "読み仮名_ruby不使用" | "読み仮名 ruby不使用":
                    # 安全获取 Ruby 文本
                    t1 = str(template.get(1).value) if template.has(1) else ""
                    t2 = str(template.get(2).value) if template.has(2) else ""
                    template_plain_text = f"{t1}({t2})" if t2 else t1
                case "!":
                    template_plain_text = "|"
                case "補助漢字フォント" | "JIS2004フォント":
                    val = str(template.get(1).value) if template.has(1) else ""
                    template_plain_text = html.unescape(val) if "&#" in val else val
                case "lang" | "Lang":
                    template_plain_text = str(template.get(2).value) if template.has(2) else (str(template.get(1).value) if template.has(1) else "")
                case "Harvnb" | "Harvnb ":
                    t1 = str(template.get(1).value) if template.has(1) else ""
                    t2 = str(template.get(2).value) if template.has(2) else ""
                    template_plain_text = t1 + t2
        except Exception:
